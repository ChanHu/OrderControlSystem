...
public class CustomerDao implements ICustomerDao{
	...
	public boolean modifyCustomer(Customer Customer) {
		...
		try {
			connection=DBUtil.getConnection();
			String sql="update Customer set CustomerID=?,CustomerName=?, CustomerAddress=?, CustomerContacts=?, CustomerPhone=? where Customerid =? ";
			PreparedStatement pStatement=connection.prepareStatement(sql);
			pStatement.setString(1, Customer.getCustomerID());
			pStatement.setString(2, Customer.getCustomerName());
			pStatement.setString(3, Customer.getCustomerAddresss());
			pStatement.setString(4, Customer.getCustomerContacts());
			pStatement.setString(5, Customer.getCustomerPhone());
			pStatement.setString(7, Customer.getCustomerID());
			f=pStatement.execute();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public List<Customer> searchCustomer(String CustomerID,String CustomerName) {
		...
		try {
			connection=DBUtil.getConnection();
			String sql="select * from Customer where CustomerID like ? or CustomerName like ?";
